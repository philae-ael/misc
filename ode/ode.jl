...
# ╔═╡ c0f68462-185c-11f0-223f-efe30ef36d8b
begin 
	using Markdown
	using PlutoUI
	using Plots
...
# ╔═╡ 2dd71948-4f95-4dab-a533-3f2625acc08d
md"""
# ODEs
"""

# ╔═╡ cba60760-265e-4dc9-9691-9c598f99a08f
md"""
## Using plot.jl and DifferentialEquations.jl
"""

# ╔═╡ 07da9107-2c7c-4b5b-b2cf-88e4d2552e1f
...
# ╔═╡ 14dd0942-c452-49af-ab66-50e2f28406c1
begin
	local u0 = [1.0;0.0;0.0]
	local tspan = (0.0,25.0)
	local prob = ODEProblem(lorenz, u0, tspan)
	sol0 = solve(prob)
end

# ╔═╡ 17847af7-9dca-4d0e-81c8-b3778f81bb51
plot(sol0, idxs=(1,2,3))

# ╔═╡ 4d4710df-6be7-4b7c-8124-88008420b6a9
md"""
## Euler method
"""

# ╔═╡ cb7940d1-8c2e-4b0e-9c62-cdc7a15e1908
md"""
Given an equa diff
```math
y'(x) = f(y(x), x)
...
The Euler method gives an approximation of $f$:
```math
y_{n+1} = y_{n} + f(y_n) (x_{n+1} - x_n) 
```

A first exemple:
Let $f = \frac{y - 2xy}{1+ x}$, $y_0 = \frac 2 5$.
"""

# ╔═╡ 89caed55-0c36-4b55-b619-cf1f372cae94
function euler_fixed_step(f, h, y0, x0, N::Int)
	xns = [x0]
	yns = [y0]
...
# ╔═╡ 5969151a-2c63-4dc6-8e53-d3dab3142b7a
begin
	local f1(y, x) = (y - 2*x*y) / (1 + x)
	local y0 = 2/5
	local x0 = 0.0
	local h = 1/10
	local N = 10
	sol1 = euler_fixed_step(f1, h, y0, x0, N+1)
end

# ╔═╡ a21f60d2-456b-4fff-9b76-a5196f6b5b1a
begin
	grayish=RGBA(0.2,0.2,0.2,0.2)
	local p = plot(sol1[1], sol1[2], markershape=:circle, markercolor="white")
	local x = range(0, 1, length=100)
	local c = range(0, 1, length=50)
	
	local solf1(x, c) = (1 + x) / (c + x^2)
	local y = [solf1.(x, 1/ci) for ci in c]
	plot!(p, x, y, color=grayish, legend=false)
end

# ╔═╡ 4a263ab9-e46b-4d14-a2d8-5d0d23ac4597
begin
	local f2(y, x) = (y+x)/(y-x)
	local y0 = 1.0
	local x0 = 0.0
	local xmax = 0.5
	local h = 0.025
	local N = Int( floor((xmax - x0)/h) )
	sol2 = euler_fixed_step(f2, h, y0, x0, N+1)
end

# ╔═╡ d0d281b6-d2a5-4250-af0a-973e3116a19c
begin
	local p = plot(sol2[1], sol2[2], markershape=:circle, markercolor="white")
	local x = range(0, 0.5, length=100)
	
	local solf2(x) = x + sqrt(1 + 2x^2)
	local y = solf2.(x)
	plot!(p, x, y, color=grayish, legend=false)
end

# ╔═╡ bab670dd-d44b-4fc2-9e64-96ab8f0bb502
# See https://math.stackexchange.com/a/4582092
function euler_adaptative_step(f, h, y0, x0, xf, tolerance)
	xns = [x0]
	yns = [y0]
	...
	while xn < xf
		dy2 = f(yn + h*dy, xn+h)
		err = 0.5*h*norm(dy - dy2)
		fac = max(0.5, min(2, tolerance/err))
		if fac >= 1
			yn += h*dy
			dy = dy2
			xn += h
...
# ╔═╡ 91d6d31a-e779-4bee-8de1-7bdedcf51795
begin
	local f3(y, x) = (y+x)/(y-x)
	local y0 = 1.0
	local x0 = 0.0
	local xf = 0.5
	local h = 0.05
	sol3 = euler_adaptative_step(f3, h, y0, x0, xf, 0.001)
	
	local p = plot(sol3[1], sol3[2], markershape=:circle, markercolor="white", idxs=(1,2))
	plot!(sol2[1], sol2[2], markershape=:circle, markercolor="white", idxs=(1,2))
	local x = range(x0, xf, length=100)
	local solf3(x) = x + sqrt(1 + 2x^2)
	local y = solf3.(x)
	plot!(p, x, y, color=grayish, legend=false)
end

# ╔═╡ f7dd7131-3e43-4803-8ddb-68a5f8ad9905
@bind T4 Slider(0.001:0.001:1, show_value=true, default=0.05)

# ╔═╡ 39222dce-cee6-4aea-8bc2-eea48a53fbbc
begin
	f4(y, x) = [
		-16y[1] + 12y[2] + 16cos(x) - 13sin(x)
		 12y[1] -  9y[2] - 11cos(x) +  9sin(x)
	]
	local y0 = [0.0, 1.0]
	local x0 = 0.0
	local xf = π
	local h = π/5
	sol4 = euler_adaptative_step(f4, h, y0, x0, xf, T4)
	local p = plot(sol4[1], transpose(hcat(sol4[2]...)))
	
	local x = range(x0, xf, length=100)	
	solf4(t) = [cos(t) sin(t)]
	plot!(p, x, vcat(solf4.(x)...), color=grayish, legend=false)
end

# ╔═╡ ced471b1-4561-43b6-aee4-8bea372cb9cc
@bind N5 Slider(1:1:100, show_value=true, default=40)

# ╔═╡ 1d2d6029-89ff-48d2-bb06-402e12ce2ebd
begin
	local y0 = [0.0, 1.0]
	local x0 = 0.0
	local xf = π
	local N = N5
	local h = π/N
	sol5 = euler_fixed_step(f4, h, y0, x0, N+1)
	local p = plot(sol5[1], transpose(hcat(sol5[2]...)))
...
# ╔═╡ 7fab2d7b-eae0-4e34-ad85-22301d807c4a
begin
	local s = transpose(hcat(sol5[2]...))
	p = plot(s[:,1],s[:,2])
	local s = transpose(hcat(sol4[2]...))
	plot!(p, s[:,1],s[:,2])
end
...
manifest_format = "2.0"
project_hash = "70212c4a04ac53f44b1c5b9d4c800b983206fff0"
